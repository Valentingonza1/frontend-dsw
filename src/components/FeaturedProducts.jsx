...
export default function FeaturedProducts() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // si tu backend soporta destacados+limit, genial:
        // const rows = await api("/productos?destacados=1&limit=4");
        // fallback robusto: cargo y corto a 4 del lado del cliente
  ...
  return (
    <section className="home-section like-steps fp" style={{ maxWidth: 1200, margin: "0 auto", padding: "0 16px" }}>
      <div className="steps-head">
        <h2 className="steps-title">PRODUCTOS DESTACADOS</h2>
        <a className="steps-link" href="/productos">Ver más</a>
      </div>

      {loading && <p className="steps-msg">Cargando…</p>}
      {msg && !loading && <p className="steps-msg">{msg}</p>}
      {!loading && !items.length && <p className="steps-msg">Por ahora no hay destacados.</p>}
            ...
            <article className="steps-card" key={p.id}>
              ...
              <div className="steps-body">
                ...
                <div className="steps-meta">
                  {p.precio != null && <span className="steps-price">{money(Number(p.precio) || 0)}</span>}
                  {"stock" in p && (
